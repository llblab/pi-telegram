...
- `/telegram-connect` — start polling here and acquire external Telegram control ownership. Accepted queue/reply state stays local if ownership later moves elsewhere. A successful command queues a hidden connection-state note for delivery with the agent's next turn without triggering one; it says Telegram is connected and that connectivity alone is not user intent. In Threaded Mode, `/telegram-connect as=Flightprice` requests that unique operator tab name for a new binding; `/telegram-connect work as=Flightprice` still selects named profile `work` first. Requested names must be one capitalized Latin word and are rejected when another live tab already uses them, in which case provisioning bakes the next unused palette name.
- `/telegram-name` — rename this instance's current Threaded Mode tab. The name must be one unique capitalized Latin word; occupied live names and invalid labels fail closed without calling Telegram.
