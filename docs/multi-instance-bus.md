...
- Instance-thread names are short, recognizable, and unique among live tabs. Default provisioning chooses one baked 4-6 letter single-word Latin thread name from the assigned slot's five-name palette using provisioning timestamp entropy, skipping names already used by current live/starting/pending bindings, and creates the Telegram thread with that title immediately. If that slot's five names are taken, provisioning continues through later palettes rather than duplicating a live title. Operators may request a name with `/telegram-connect as=Name` or rename the current tab with `/telegram-name Name`; both still require one capitalized Latin word and fail closed on a live duplicate. The slot remains internal ordering metadata and is not redundantly included in the thread name. Bare slot titles are fallback/legacy state only; do not prompt agents to self-name. Existing human-named threads are preserved across reloads and leadership changes when they remain the current live binding. If reload creates a new runtime instance while the previous leader thread is still alive, the new leader should take the next free slot instead of reusing the old slot immediately.
