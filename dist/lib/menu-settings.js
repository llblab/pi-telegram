...
function getTelegramSettingsStateValueLabel(value) {
    return value.toLowerCase();
}
function getVoiceReplyModeSetting(mode, configured) {
    return configured ? mode : "manual";
}
export function buildTelegramSettingsMenuText() {
    return SETTINGS_MENU_TITLE;
}
export function buildThreadDisplaySettingsText(mode, custom = false) {
    return [
...
export function buildAssistantRenderingSettingsText(mode) {
    return [
...
export function buildActivityVerbositySettingsText(verbosity) {
    return [
...
export function buildVoiceReplyModeSettingsText(mode, configured = true) {
    return [
        `${VOICE_REPLY_MODE_SETTINGS_TITLE} <code>${getVoiceReplyModeLabel(getVoiceReplyModeSetting(mode, configured))}</code>`,
        "",
...
export function buildTimeInjectionModeSettingsText(mode) {
    return [
...
export function buildTelegramSettingsMenuReplyMarkup(draftPreviewsEnabled, assistantRenderingModeOrVoiceReplyMode, voiceReplyModeOrTimeInjectionMode, timeInjectionModeOrSectionRegistry, sectionRegistryOrVoiceReplyModeConfigured, voiceReplyModeConfigured = true, automaticThreadCleanupEnabled = true, activityVerbosity = "quiet", threadDisplayMode, threadDisplayCustom = false) {
    const hasRenderingMode = assistantRenderingModeOrVoiceReplyMode === "rich" ||
        assistantRenderingModeOrVoiceReplyMode === "html";
    const assistantRenderingMode = hasRenderingMode ? assistantRenderingModeOrVoiceReplyMode : "rich";
    const voiceReplyMode = hasRenderingMode
        ? voiceReplyModeOrTimeInjectionMode
    ...
    const settingsButtons = [
        ...
        {
            text: `👄 Voice reply: ${getTelegramSettingsStateValueLabel(getVoiceReplyModeLabel(getVoiceReplyModeSetting(voiceReplyMode, effectiveVoiceReplyModeConfigured)))}`,
    ...
    if (sectionRegistry) {
        ...
        settingsButtons.push(...extensionRows.map((row) => ({
            text: row.label,
...
export function buildDraftPreviewsSettingsReplyMarkup(enabled) {
    return {
        inline_keyboard: [
            [{ text: "⬆️ Back", callback_data: "settings:list" }],
            [
...
export function buildAssistantRenderingSettingsReplyMarkup(mode) {
    const modes = ["rich", "html"];
    return {
        inline_keyboard: [
            [{ text: "⬆️ Back", callback_data: "settings:list" }],
            ...modes.map((value) => [
...
export function buildActivityVerbositySettingsReplyMarkup(verbosity) {
    const button = (value) => ({
...
export function buildTimeInjectionModeSettingsReplyMarkup(mode) {
    const modes = ["hidden", "always", "interval"];
    return {
        inline_keyboard: [
            [{ text: "⬆️ Back", callback_data: "settings:list" }],
            ...modes.map((value) => [
...
export function buildVoiceReplyModeSettingsReplyMarkup(mode, configured = true) {
    const activeMode = getVoiceReplyModeSetting(mode, configured);
    const modes = ["manual", "mirror", "always"];
    return {
        inline_keyboard: [
            [{ text: "⬆️ Back", callback_data: "settings:list" }],
            ...modes.map((value) => [
...
export async function handleTelegramSettingsMenuCallbackAction(callbackQueryId, data, deps) {
    ...
    if (data === "settings:open:time-injection" ||
    ...
    if (data.startsWith("settings:set:voice-reply:")) {
        ...
        if (mode === "manual" ||
            ...
            await updateVoiceReplyModeSettingsMessage(deps);
            await deps.answerCallbackQuery(callbackQueryId, `Voice reply mode: ${normalizedMode}`);
            return true;
        }
    }
    if (data.startsWith("settings:set:time-injection:") ||
        ...
        if (normalizedMode === "hidden" ||
            ...
            await updateTimeInjectionModeSettingsMessage(deps);
            await deps.answerCallbackQuery(callbackQueryId, `Time injection: ${normalizedMode}`);
            return true;
        }
    }
    if (data === "settings:set:draft-previews:on" ||
        ...
        await updateDraftPreviewsSettingsMessage(deps);
        await deps.answerCallbackQuery(callbackQueryId, `Draft previews ${enabled ? "enabled" : "disabled"}`);
        return true;
    }
    ...
    if (data === "settings:set:automatic-thread-cleanup:on" ||
        data === "settings:set:automatic-thread-cleanup:off") {
        const enabled = data.endsWith(":on");
        await deps.setAutomaticThreadCleanupEnabled(enabled);
        await updateAutomaticThreadCleanupSettingsMessage(deps);
        await deps.answerCallbackQuery(callbackQueryId, `Thread cleanup ${enabled ? "enabled" : "disabled"}`);
        return true;
    }
    await deps.answerCallbackQuery(callbackQueryId);
    return true;
