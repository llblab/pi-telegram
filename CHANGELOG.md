...
- `Workspace ownership`: Verified detachment records first inactivity without clearing work; preserved Threads retain binding and letter. Quit requires quiescence and excludes reload/new/resume/fork. Dead-follower proof is registration/profile/epoch fenced; replacement cancels it. Leader startup waits one follower-staleness window before contracting directory suffixes, preventing election/re-registration flicker while still removing a suffix after the roster settles.
- `Workspace rejection recovery`: Proven Telegram rejection now cancels pressure retirement without deleting its binding or permanently blocking admission. Durable rejection survives interrupted intent withdrawal and fence release; recovery finishes cancellation without another delete. Fresh attempts use distinct authority, while network/5xx/malformed or mismatched outcomes remain fenced without replay.
- `Storage path authority`: Exact-path preflight rejects aliases without repair. Bounded inventory covered every current/retained CWD hint and the sole known profile; no defective-path source was present. Dedicated profile-only polling/admission resolvers now bind the configured agent directory, preventing profile names from becoming CWD-relative roots. Composition invariants reject the former callback shape; retained canonical v1 queues remain untouched.
- `Worker, follower and answer readiness`: Prepared workers renew expired capabilities; retained access holds references. Follower admission requires current registration, context and session preparation; stale replies cannot restore stopped work or erase newer authority. Post-await fences protect completion and media enqueue. An exact Telegram intermediate answer now suppresses identical final settlement, preventing duplicate persistent replies. Production custody stays disabled.
- `Receipt and handoff authority`: V3 dispatch and owner lookup validate current custody; settlement requires an exact completion ACK, not lost readiness. Confirmed receipts allow local cleanup without replay. Control reconciliation uses the serial payload boundary rather than cloning executable callbacks, preserving recipient-local execution and donor retention on rejection/lost ACK. Native tests cover the actual reconciler and wire codec; custody activation stays gated.
- `Pending queue safety`: Default routing ignores Business deletions outside the bot-chat namespace; raw handlers still observe them. Workers retain newer wakes without clearing later failures or replaying unknown outcomes. Only immutable exclusions bypass mutation guards. Accepted work progresses after transport loss; prepared v3 workers drain excluded pending input after restoration/restart through veto-only removal, never raw completion. Custody activation stays gated.
- `Installation and session docs`: Aligns the documented Pi minimum with package peers and describes the supported Telegram `/new` path without implying general remote session navigation.
