...
- When all `A`–`Z` slots are reserved, a fresh connection rotates the oldest proven inactive, unprotected binding. This deletes the old Telegram Thread **and all its messages**, then reuses its letter; Pi session history, project files, memory, and journals are not deleted. Unfinished work normally blocks rotation. Only complete unoffered receipt groups whose exact process-birth owners are all proven dead may be discarded without replay under actual slot pressure, followed by a fresh protection check. Live/unverifiable owners, partial groups, handoffs, incomplete sources, and unknown safety evidence remain protected. Restoring a remembered session does not evict another binding. With automatic cleanup disabled, verified leader quit or confirmed-dead follower detachment retains the tab and records inactivity for later pressure rotation; reload/new/resume/fork never detach it. Within the same leader runtime, pruned followers can be rechecked for later PID absence and failed inactivity writes retried without restoring their live routing.
